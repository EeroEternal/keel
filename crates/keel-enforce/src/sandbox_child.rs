...
pub fn apply_policy_file_and_ready(path: &Path, block_process_network: bool) -> EnforceResult<Policy> {
...
pub fn write_spawn_policy_file(policy: &Policy) -> EnforceResult<std::path::PathBuf> {
    ...
    let path = dir.join(format!("policy-{}-{}.json", policy.id.as_str(), std::process::id()));
